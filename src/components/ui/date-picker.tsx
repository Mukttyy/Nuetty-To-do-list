...
export function DatePicker({ value, onChange }: { value?: string; onChange: (value?: string) => void }) {
  ...
  return <>
    ...
    <Dialog open={open} title="Choose due date" onClose={() => setOpen(false)} className="sm:max-w-sm max-h-[95dvh] overflow-y-auto">
      ...
      <div className="mt-2 grid grid-cols-7 gap-1 text-center text-xs">
        {["Su", "Mo", "Tu", "We", "Th", "Fr", "Sa"].map(day => <span key={day} className="py-2 text-zinc-500">{day}</span>)}
        ...
        {Array.from({ length: days }, (_, i) => {
          ...
          return <button key={date} type="button" aria-label={date} aria-pressed={value === date} aria-current={today === date ? "date" : undefined} onClick={() => choose(date)} className={`min-h-10 rounded-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-zinc-500 ${value === date ? "bg-zinc-900 text-white" : "text-zinc-800 hover:bg-zinc-100"} ${today === date ? "font-bold underline underline-offset-4" : ""}`}>{i + 1}</button>;
        })}
      </div>
      <div className="mt-4 flex justify-between border-t border-zinc-200 pt-3 text-sm">
